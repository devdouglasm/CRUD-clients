package com.devdouglasm.crud_clients.controllers.handlers;

import com.devdouglasm.crud_clients.dto.CustomError;
import com.devdouglasm.crud_clients.dto.ValidationError;
...
public class ControllerExceptionHandler {
    ...
    public ResponseEntity<CustomError> resourceNotFound(ResourceNotFoundException e, HttpServletRequest request) {
        HttpStatus status = HttpStatus.NOT_FOUND; //        .value in status to bring the int
    ...
    public ResponseEntity<CustomError> resourceNotFound(DatabaseException e, HttpServletRequest request) {
        HttpStatus status = HttpStatus.NOT_FOUND; //        .value in status to bring the int
        CustomError error = new CustomError(Instant.now(), status.value(), "Invalid data", request.getRequestURI());
        return ResponseEntity.status(status).body(error);
    }
